...
export default function Home() {
    ...
    const queueSong = async (event: any) => {
        ...
        if (e.classList.contains(styles.queueing) || e.classList.contains(styles.success)) return;
                ...
                <div className={styles.bar}>
                    <form className={styles.search} onSubmit={(e) => e.preventDefault()}>
                        ...
                        <div className={styles.clear} onClick={clearSearch}>
                            {results.length > 0 && (
                                <Image width={30} height={30} src="/xmark-solid.svg" alt="xmark" />
                            )}
                        </div>
                    </form>
                            ...
                            {state.name && (
                                <>
                                    ...
                                    <div className={styles.meta}>
                                        <h4>{state.name}</h4>
                                        <span>{state.artists.map((a) => a.name).join(", ")}</span>
                                    </div>
                                </>
                            )}
                        </div>
                    ...
                    <div className={styles.scan}>
                        {state.name && (
                            ...
                            <img
                                src={`https://scannables.scdn.co/uri/plain/jpeg/C6C0B4/black/256/${state.uri}`}
                                alt="spotify scan"
                            />
                        )}
                    </div>
                    ...
                    {results.map((result: Track) => {
                        return (
                            ...
                            >
                                ...
                                <div className={styles.info}>
                                    <h3>{result.name}</h3>
                                    <h5>{result.artists.map((a) => a.name).join(", ")}</h5>
                                </div>
